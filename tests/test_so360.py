"""360 Search (so.com) adapter test.

What it checks:
1. Run So360Engine._do_search("AI", limit=5) with up to 3 attempts
   (bypasses BaseEngine.search retry loop so each attempt's
   diagnostics are visible — same pattern as test_sogou.py /
   test_baidu.py).
2. After every attempt, print:
     - the page title / URL,
     - selector counts for the cross-layout result-row selectors,
...
3. PASS if at least one result is returned (assert len(results) > 0).
   ...
   nothing.
4. Print the top 5 results with title / url / source / snippet.

Browser is launched with locale=zh-CN and timezone=Asia/Shanghai so
360 serves its standard Chinese desktop layout — the en-US gateway
sometimes routes through a different layout with different markup.

Run:
    source ~/tools/cloakbrowser/venv/bin/activate
    cd /Users/gao/projects/AgentSearch
    python tests/test_so360.py
"""
...
# of `cloak_stealth_suite` that might be registered in site-packages, which
# could lack this repo's `core` module and break the import below.
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from cloak_stealth_suite import core
from cloak_stealth_suite.engines.so360 import So360Engine
...
def _attempt(engine: So360Engine, attempt: int) -> list:
...
def main() -> int:
    ...
    print("=== 360 Search (so.com) adapter test ===")
    ...
    try:
        ...
        # Required assertion.
        assert len(results) > 0, "expected at least one 360 search result"
