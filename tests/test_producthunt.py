"""Product Hunt search adapter test.

What it checks:
1. Run ProductHuntSearchEngine._do_search("AI tools", limit=5) with up
   to 3 attempts (bypasses BaseEngine.search retry loop so each
   attempt's diagnostics are visible — same pattern as test_medium.py
   / test_quora.py).
2. After every attempt, print:
     - the page title / URL,
     - which mode was used (producthunt_direct / ddg_site),
     - selector counts for the cross-mode selectors so a parsing
       miss is obvious from a single dump,
     - any block_reason / selector / count flag on
       engine.last_status,
     - check_blocked() reason if any.
3. PASS if at least one product is returned (assert len(results) > 0).
   FAIL with diagnostics if every attempt is blocked or returns no
   results.
4. Print the top 5 results with name / url / tagline / upvotes /
   reviews / categories.

Run:
    source ~/tools/cloakbrowser/venv/bin/activate
    cd /Users/gao/projects/AgentSearch
    python tests/test_producthunt.py
"""
...
from cloak_stealth_suite.engines.producthunt import ProductHuntSearchEngine
...
def _attempt(engine: ProductHuntSearchEngine, attempt: int) -> list:
...
def main() -> int:
    ...
    print("=== Product Hunt search adapter test ===")
    print(
    ...
    try:
        ...
        engine = ProductHuntSearchEngine(page)
        ...
        if not results:
            print(
                "\n=== FAIL === no products after all attempts "
                "(producthunt_direct + ddg_site both empty)",
                file=sys.stderr,
            )
            return 1

        # Required assertion.
        assert len(results) > 0, "expected at least one Product Hunt result"

        print(f"\nReturned {len(results)} results")
        print("\n--- Top 5 results ---")
        for i, r in enumerate(results[:5], start=1):
            tagline = getattr(r, "tagline", "") or ""
            upvotes = getattr(r, "upvotes", None)
            upvotes_str = f"{upvotes:,}" if upvotes is not None else "n/a"
            reviews = getattr(r, "reviews", None)
            ...
            print(f"    URL       : {r.url}")
            if tagline:
                t_show = tagline.replace("\n", " ")
                if len(t_show) > 200:
                    t_show = t_show[:200] + "..."
                print(f"    Tagline   : {t_show}")
            print(f"    Upvotes   : {upvotes_str}")
            print(f"    Reviews   : {reviews_str}")
            print(f"    Categories: {cats_str}")
            if r.snippet and r.snippet != tagline:
