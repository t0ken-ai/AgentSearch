"""Wikivoyage search adapter smoke test.

Steps:
1. Launch headless browser via core.launch().
2. Run WikivoyageEngine.search("Tokyo").
...
Run:
    ...
    python tests/test_wikivoyage.py
"""
...
from cloak_stealth_suite.engines.wikivoyage import WikivoyageEngine
...
def main() -> int:
    ...
    print("=== Wikivoyage search adapter test ===")
    ...
    try:
        ...
        engine = WikivoyageEngine(page)

        results = engine.search(QUERY, limit=LIMIT)

        print(f"\nReturned {len(results)} results")
        assert len(results) > 0, "expected at least one Wikivoyage result"
