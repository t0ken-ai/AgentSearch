"""Baidu search adapter test.

What it checks:
1. Run BaiduEngine._do_search("人工智能", limit=5) with up to 3 attempts
   (bypasses BaseEngine.search retry loop so each attempt's diagnostics
   are visible — same pattern as test_bing.py / test_yandex.py).
...
Baidu serves its standard Chinese desktop layout — the en-US gateway
sometimes routes through the simplified mobile / international page,
which has different markup.
...
def _attempt(engine: BaiduEngine, attempt: int) -> list:
...
def main() -> int:
    ...
    print("=== Baidu search adapter test ===")
    ...
    try:
        ...
        # Required assertion.
        assert len(results) > 0, "expected at least one Baidu result"
