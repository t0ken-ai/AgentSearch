"""Startpage adapter test.

What it checks:
1. Run StartpageEngine._do_search("privacy tools", limit=5) with up to
   3 attempts (bypasses BaseEngine.search retry loop so each attempt's
   diagnostics are visible — same pattern as test_brave.py /
   test_so360.py).
2. After every attempt, print:
     - the page title / URL,
     - selector counts for the cross-layout result-card selectors,
...
3. PASS if at least one result is returned (assert len(results) > 0).
   ...
   nothing.
4. Print the top 5 results with title / url / display_url / snippet.

Run:
    source ~/tools/cloakbrowser/venv/bin/activate
    cd /Users/gao/projects/AgentSearch
    python tests/test_startpage.py
"""
...
# install of `cloak_stealth_suite` that might be registered in
# site-packages (e.g. the stale copy at
# /Users/gao/projects/cloak-stealth-suite/), which would lack this
# repo's `engines.startpage` module and break the import below.
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from cloak_stealth_suite import core
from cloak_stealth_suite.engines.startpage import StartpageEngine
...
def _attempt(engine: StartpageEngine, attempt: int) -> list:
...
def main() -> int:
    ...
    print("=== Startpage search adapter test ===")
    ...
    try:
        ...
        # Required assertion.
        assert len(results) > 0, "expected at least one Startpage result"
