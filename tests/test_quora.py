"""Quora search adapter test.

What it checks:
1. Run QuoraSearchEngine._do_search("best programming language 2025",
   ...
   test_medium.py / test_blackhatworld.py).
2. After every attempt, print:
     - the page title / URL,
     - which mode was used (quora_direct / google_site / ddg_site),
     - selector counts for the cross-mode selectors so a parsing
       miss is obvious from a single dump,
     - any block_reason / selector / count / modal_dismissed flag on
       engine.last_status,
     - check_blocked() reason if any.
3. PASS if at least one Q&A is returned (assert len(results) > 0).
   FAIL with diagnostics if every attempt is blocked or returns no
   results.
4. Print the top 5 results with title / url / kind / author / upvotes /
   answer preview.
...
from cloak_stealth_suite.engines.quora import QuoraSearchEngine
...
def _attempt(engine: QuoraSearchEngine, attempt: int) -> list:
    ...
    if engine.last_status:
        ...
        if ls.get("modal_dismissed") is not None:
            print(f"  modal dismissed: {ls.get('modal_dismissed')}")
        if ls.get("error"):
...
def main() -> int:
    ...
    print("=== Quora search adapter test ===")
    print(
    ...
    try:
        ...
        if not results:
            print(
                "\n=== FAIL === no Q&A after all attempts "
                "(quora_direct + google_site + ddg_site all empty)",
                file=sys.stderr,
            )
            return 1

        # Required assertion.
        assert len(results) > 0, "expected at least one Quora result"

        print(f"\nReturned {len(results)} results")
        print("\n--- Top 5 results ---")
        for i, r in enumerate(results[:5], start=1):
            kind = getattr(r, "kind", "") or "(unknown)"
            ...
            if answer:
                ...
                if len(ans) > 240:
                    ans = ans[:240] + "..."
                print(f"    Answer  : {ans}")
            if r.snippet and r.snippet != answer:
