...
What it checks:
1. Run GoogleEngine.search("open source software") with up to 3 attempts.
2. After every attempt, print the count for every result selector
   (div.g / .tF2Cxc / data-sokoban-container / div.MjjYud / #search h3 / #rso h3).
3. If the page got blocked / shows a CAPTCHA / sorry interstitial, print the
...
def _attempt(engine: GoogleEngine, attempt: int) -> list:
...
def main() -> int:
    ...
    print("=== Google search adapter test ===")
