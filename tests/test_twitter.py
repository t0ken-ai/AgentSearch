"""Twitter/X search adapter test.

What it checks:
1. Run TwitterEngine._do_search("AI", limit=5) with up to 3 attempts.
2. After every attempt, print:
     - the page title / URL,
     - which mirror / mode was tried (last_status),
     - selector counts for both Nitter (.timeline-item, .tweet-content,
       .tweet-link, .username) and x.com (article[data-testid="tweet"],
       [data-testid="tweetText"]),
     - any block reason set on engine.last_status.
3. PASS if at least one tweet is returned (assert len(results) > 0).
   FAIL with diagnostics if every Nitter mirror is dead AND x.com only
   shows the login wall.
4. Print the top 5 tweets with title / url / score / snippet.
...
def _attempt(engine: TwitterEngine, attempt: int) -> list:
    ...
    if engine.last_status:
        ...
        if ls.get("mirror"):
            print(f"  last mirror    : {ls.get('mirror')}")
        if ls.get("mode"):
            print(f"  last mode      : {ls.get('mode')}")
...
def main() -> int:
    ...
    print("=== Twitter/X search adapter test ===")
    ...
    try:
        ...
        if not results:
            print(
                "\n=== FAIL === no tweets after all attempts "
                "(every Nitter mirror dead and x.com required login)",
                file=sys.stderr,
            )
            return 1

        # Required assertion.
        assert len(results) > 0, "expected at least one tweet"
