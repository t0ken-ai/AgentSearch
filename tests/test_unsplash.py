"""Unsplash search adapter test.

Run:
    source ~/tools/cloakbrowser/venv/bin/activate
    cd /Users/gao/projects/AgentSearch
    python tests/test_unsplash.py
"""

from __future__ import annotations

import logging
import os
import sys
import traceback

_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from cloak_stealth_suite import core
from cloak_stealth_suite.engines.unsplash import UnsplashEngine


QUERY = "mountain landscape"
LIMIT = 5


def main() -> int:
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
    )

    print("=== Unsplash search adapter test ===")
    print(f"Query: {QUERY!r} | Limit: {LIMIT}")

    cfg = core.BrowserConfig(headless=True, humanize=True)
    browser = core.launch(cfg)
    try:
        page = core.new_page(browser)
        engine = UnsplashEngine(page)
        results = engine.search(QUERY, limit=LIMIT)

        ls = engine.last_status or {}
        print(f"\nFigures seen: {ls.get('figures_seen', 0)}")

        if not results:
            print("\n=== FAIL === no results", file=sys.stderr)
            return 1

        assert len(results) > 0, "expected at least one Unsplash result"

        print(f"\nReturned {len(results)} results")
        print("\n--- Top 5 results ---")
        for i, r in enumerate(results[:5], start=1):
            photographer = getattr(r, "photographer", "") or ""
            photographer_username = getattr(r, "photographer_username", "") or ""
            photo_id = getattr(r, "photo_id", "") or ""
            image_url = getattr(r, "image_url", "") or ""
            alt = getattr(r, "alt_text", "") or ""
            print(f"\n[{i}] {r.title}")
            print(f"    URL    : {r.url}")
            if image_url:
                print(f"    Image  : {image_url[:120]}")
            meta = []
            if photographer:
                meta.append(f"by {photographer}")
            if photographer_username:
                meta.append(f"@{photographer_username}")
            if photo_id:
                meta.append(f"id={photo_id}")
            if meta:
                print(f"    Meta   : {' · '.join(meta)}")
            if alt:
                a = alt.replace("\n", " ")
                print(f"    Alt    : {a[:160]}")

        print("\n=== PASS ===")
        return 0
    except AssertionError as e:
        print(f"\n=== FAIL === assertion: {e}", file=sys.stderr)
        return 1
    except Exception:
        print("\n=== FAIL === unexpected exception:", file=sys.stderr)
        traceback.print_exc()
        return 1
    finally:
        try:
            browser.close()
        except Exception as e:
            print(f"warning: browser.close() raised: {e}", file=sys.stderr)


if __name__ == "__main__":
    sys.exit(main())
