"""Zhihu (知乎) search adapter test (with Google site: fallback).
...
def main() -> int:
    ...
    print("=== Zhihu search adapter test ===")
    ...
    try:
        ...
        assert len(results) > 0, "expected at least one Zhihu result"

        print(f"\nReturned {len(results)} results")
        print("\n--- Top 5 results ---")
        for i, r in enumerate(results[:5], start=1):
            ctype = getattr(r, "content_type", "") or ""
            zid = getattr(r, "zhihu_id", "") or ""
            author = getattr(r, "author", "") or ""
            voteup = getattr(r, "voteup_count", "") or ""
            comment = getattr(r, "comment_count", "") or ""
            ...
            if ctype:
                meta.append(ctype)
            if zid:
                meta.append(f"id={zid}")
            if author:
                meta.append(f"by {author}")
            if voteup:
                meta.append(f"赞同 {voteup}")
            if comment:
                meta.append(f"评论 {comment}")
            if source:
