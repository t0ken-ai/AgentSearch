"""Sogou search adapter test.

What it checks:
1. Run SogouEngine._do_search("Python 教程", limit=5) with up to 3
   attempts (bypasses BaseEngine.search retry loop so each attempt's
   diagnostics are visible — same pattern as test_baidu.py /
   test_bing.py).
...
Sogou serves its standard Chinese desktop layout — the en-US gateway
...
from cloak_stealth_suite.engines.sogou import SogouEngine
...
def _attempt(engine: SogouEngine, attempt: int) -> list:
...
def main() -> int:
    ...
    print("=== Sogou search adapter test ===")
    ...
    try:
        ...
        # Required assertion.
        assert len(results) > 0, "expected at least one Sogou result"
