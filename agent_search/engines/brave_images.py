"""Brave Images search adapter.

Brave's image SERP is rendered server-side in tile cards. We parse
the rendered DOM with a generic-but-effective image extractor.
"""
...
class BraveImagesEngine(ImageSearchEngine):
    name = "brave_images"
    max_retries = 1

    def _do_image_search(self, query, limit):
        q = urllib.parse.quote(query)
        url = f"https://search.brave.com/images?q={q}&source=web"
